...
def chemical_potential(elements_name,folder_elements, deltamiu):
    ...
    for i in range(len(elements_name)):
        ...
        miu_i = np.round(miu_bulk + deltamiu[ele_i],3) # bulk/molecule_energy + delta_miu; round to 3 decimals
...
class readformationenergy:
    def __init__(self,folder, folder_perfect, list_defect, miu_elements, comment='',folder_bandgap=None):
        if folder[-1] != '/':
            folder = folder + '/'
        self.folder=folder
        assert folder_perfect[0] != '/', 'the perfect folder needs to start from 2nd-class'
        self.hier1direc =os.environ['PWD'] + '/'# current folder Eg. '/home/yubi/work/berylliumO/'
        ...
        self.colors = ['black','tab:red','tab:orange','tab:blue','tab:green','tab:purple','tab:brown']
    ...
    def draw_lowerbound(self, defect_onetype, transition_data_fil,defectlabeltextposition=None, numdatapoints=1001, charge_seg=False,xrange=5,yrange=20):
        ...
        plt.plot(fermi_level, formation_energy, color=self.colors[0], linewidth=3)#linestyle='--', label='%s' % (defect_onetype.defecttype)
        ...
        plt.text(label_x_pos,label_y_pos,'%s' % (defect_onetype.defecttype),fontsize=26,color=self.colors[0])
    ...
    def myplot(self,ax,charge_seg,lower_bound_plot=True,xlimits=[0, 11.3],ylimits=[-20,25],defectlabeltextposition=None):
        #fig=plt.figure()
        ...
        plt.title(self.miu_elements['condition'].replace('-',' '), fontsize=28)
        #plt.legend(fontsize=20)
        plt.tight_layout()
        savename='graphdata/%s_%s_formation-energy.jpeg' % (self.miu_elements['condition'],self.comment)
